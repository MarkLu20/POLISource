	...
	*/
	...
	for(auto const& AbsoluteRoundedMarker : MarkerData.MarkerValues)
	{
		...
		if (bDrawMarkers)
		{
			...
			//A
			int32 dir = Axis == EAxis::X ? -1 : 1;

			TArray< FVector2D > Points2;
			Points2.Init(FVector2D::ZeroVector, 2);
			Points2[0][AxisIdx] = MarkerPlotSpace;
			Points2[0][OtherAxisIdx] = FixedCompBase;
			Points2[1][AxisIdx] = MarkerPlotSpace;
			Points2[1][OtherAxisIdx] = Geometry.GetLocalSize()[OtherAxisIdx] * dir;
			FSlateDrawElement::MakeLines(
				OutDrawElements,
				LayerId,
				Geometry.ToPaintGeometry(),
				...
				ChartStyle->ChartLineColor,
				false,
				ChartStyle->ChartLineThickness*1.5f
			);
